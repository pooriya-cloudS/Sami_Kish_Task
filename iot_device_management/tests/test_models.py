...
def test_user_str_and_full_name():
    ...
    """
    user = User.objects.create_user(
        email="user2@example.com", name="Full Name", password="password123"
    )
...
def test_create_telemetry_data():
    ...
    """
    user = User.objects.create_user(
        email="test2@example.com", name="User 2", password="password123"
    )
    device = Device.objects.create(
    ...
    telemetry = TelemetryData.objects.create(
        device=device, metric_name="humidity", metric_value=45.5
    )
...
def test_device_telemetry_relationship():
    ...
    """
    user = User.objects.create_user(
        email="rel@example.com", name="Rel User", password="password123"
    )
    device = Device.objects.create(
    ...
    telemetry1 = TelemetryData.objects.create(
        device=device, metric_name="temperature", metric_value=22.3
    )
    telemetry2 = TelemetryData.objects.create(
        device=device, metric_name="humidity", metric_value=55.1
    )
